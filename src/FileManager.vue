...
export default {
  ...
  mounted() {
    EventBus.$on('dbClickFile', function (){
      console.log('Emit');
      this.$emit('dbClickSelectedFile', 1);
    });
  },
  created() {
  ...
  methods: {
    ...
    fileManagerSearch(){
      let manager = this.activatedManager();
      let disk = this.selectedDisk();
      let path = '';
      let search = this.search;
      this.$store.dispatch('fm/getSearchContent', {manager, disk, path, search});
    },
    /**
     * Active manager name
